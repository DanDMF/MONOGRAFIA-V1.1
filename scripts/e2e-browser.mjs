...
// ---------- Próximo parágrafo (cartão → parágrafo integrado) ----------
await page.goto(`${BASE}/app`);
await page.fill("#new-idea", "Ideia fictícia para verificar o fluxo do cartão");
await page.getByRole("button", { name: "Criar cartão" }).click();
await page.waitForURL(/cartoes\/[0-9a-f-]{36}/);
...
await page.fill("#c-draft", "Parágrafo fictício integrado a partir do cartão.");
...
await page.getByText(/Parágrafo integrado \(revisão/).waitFor();
log("cartão: ideia → fonte com localização → excerto literal → rascunho → integrado");
await page.locator(".alert.ok").getByRole("link").first().click();
await page.waitForURL(/editor/);
const fromCard = page.locator(".ProseMirror p.from-card", { hasText: "Parágrafo fictício integrado a partir do cartão" });
...
log("parágrafo integrado com citação e localização; ligação ao cartão preservada após editar e recarregar");
...
await mc.goto(`${BASE}/app/escrita/cartoes`);
...
if (ov2 > 1) throw new Error(`deslocamento horizontal nos cartões (telemóvel): ${ov2}px`);
await mc.locator(".card-link").first().click();
...
if (ov3 > 1) throw new Error(`deslocamento horizontal na ficha do cartão (telemóvel): ${ov3}px`);
await mc.screenshot({ path: `${SHOTS}/7-telemovel-cartao.png`, fullPage: true });
